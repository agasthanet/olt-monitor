...
def get_onts(force: bool = False, olt_id: str = None, filter_pon: str = None) -> List[OnuInfo]:
    ...
    for olt in targets:
        ...
        if vendor in ("hsairpo-cli", "airpo-cli", "hs-ept", "hs-ept1004", "cli"):
            ...
            print(f"[APP] HS-EPT CLI proto={proto} port={cli_port}")
            try:
                ...
                part = []
        elif vendor in ("hioso-cli", "ha7302", "ha7302csm"):
            raw_boards = olt.get("boards") or [1]
            try:
                pons = [int(x) for x in raw_boards]
            except Exception:
                pons = [1]
            if filter_pon:
                try:
                    ...
                    pass
            cli_port = int(olt.get("cli_port") or 23)
            if cli_port == 161:
                cli_port = 23
            print(f"[APP] Hioso CLI port={cli_port} pons={pons}")
            try:
                part = fetch_hioso_cli(
                    host=olt.get("ip"),
                    username=olt.get("username") or "root",
                    password=olt.get("password") or "",
                    port=cli_port,
                    pons=pons,
                    max_onu=int(olt.get("max_onu") or 128),
...
def index():
    ...
    return render_template(
        ...
        olt_health=get_cached_health(filter_olt) if filter_olt else None,
        cache_age=cache_age,
        cache_ttl=getattr(config, "CACHE_SECONDS", 300),
...
def api_health():
    oid = request.args.get("olt") or ""
    if not oid:
        return jsonify({"ok": False, "error": "olt required"}), 400
    olt = config.get_olt(oid)
    if not olt:
        return jsonify({"ok": False, "error": "OLT tidak ditemukan"}), 404
    force = request.args.get("force") in ("1", "true", "yes")
    if force or not get_cached_health(oid):
        data = refresh_health(olt)
    else:
...
@app.route("/api/ping")
def api_ping():
    """Ping satu OLT (atau semua). ?olt=id  atau tanpa = semua."""
    oid = request.args.get("olt") or ""
    if oid:
        olt = config.get_olt(oid)
        if not olt:
            return jsonify({"ok": False, "error": "OLT tidak ditemukan"}), 404
        result = record_ping(oid, olt.get("ip") or "")
        return jsonify({"ok": True, "olt": oid, "result": result, "history": get_history(oid, 60)})
    results = ping_all_olts(config.OLTS or [])
...
def api_ping_history():
    ...
    return jsonify({
        ...
        "olt": oid,
        "last": get_last(oid),
        "history": get_history(oid, int(request.args.get("limit") or 60)),
    })
...
def start_ping_worker():
    t = threading.Thread(target=_ping_loop, name="olt-ping", daemon=True)
    t.start()
