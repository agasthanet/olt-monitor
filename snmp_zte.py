...
def fetch_all_onts(
    ...
    if vendor_setting in ("hioso",):
        ...
        if guessed == "hioso":
            chain = ["hioso", "zte", "hsairpo"]
        elif guessed == "hsairpo":
            chain = ["hsairpo", "zte", "hioso"]
        elif guessed == "zte":
            chain = ["zte", "hioso", "hsairpo"]
        else:
            # nama generik "OLT GG DUKUH" dll — coba Hioso dulu (banyak OLT murah), lalu ZTE
            chain = ["hioso", "zte", "hsairpo"]
            print("[SNMP] Auto: sysDescr generik → coba hioso → zte → hsairpo")
